...
ap_transformed <- usmap_transform(hotspot)


plot_usmap() +
  geom_point(data = ap_transformed, aes(x = lon.1, y= lat.1, size = Del),
             color = "Blue", alpha = .95) +
  labs(title = "Airports Delay",
       subtitle = "Source: Bureau of Transportation Statistics, Jan 1 to Dec 31 2018 and 2019",
       size = "Avergae Delay (Minutes)") +
